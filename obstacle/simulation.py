...
# import inverse_dynamics_PD as id
import filter
import foot_trajectory_generator as ftg
from logger import Logger
from logger_sel import Logger as Logger_sel
from logger_sel_2 import Logger as Logger_sel_2
...
class Hrp4Controller(dart.gui.osg.RealTimeWorldNode):
    def __init__(self, world, hrp4):
        ...
        reference = [(0.2, 0., 0.0)] * 5 + [(0.2, 0., -0.0)] * 10 + [(0.2, 0., 0.)] * 5 + [(0.1, 0., 0.)] * 3 + [(0.0, 0., 0.)] * 2
        ...
        self.logger_CoM_err = Logger_sel(self.initial,'com')
        self.logger_CoM_err.initialize_plot(frequency=10)

        self.logger_foot = Logger_sel_2(self.initial,'lfoot','rfoot')
        self.logger_foot.initialize_plot('Feet height',frequency=10, offset1=5, offset2=5)
    ...
    def customPreStep(self):
        ...
        # update the error wrt the desired
        self.error = self.error_tracking(self.desired, self.current)
        # if  self.time >550 and self.time < 580:
        #     force = np.array([0, -10.0, -0.0])  
        #     self.base.addExtForce(force)
        
        # if  self.time >780 and self.time < 810:
        ...
        self.logger_CoM_err.log_data(self.error)
        self.logger_CoM_err.update_plot(self.time)

        self.logger_foot.log_data(self.current)
        self.logger_foot.update_plot(self.time)
    ...
    def error_tracking(self, desired, current):
        # compute error between desired and current state
        error = {}
        for key in ['lfoot', 'rfoot', 'com', 'torso', 'base']:
            error[key] = {}
            for subkey in ['pos', 'vel', 'acc']:
                error[key][subkey] = desired[key][subkey] - current[key][subkey]
        return error
