...
class InverseDynamics:
    ...
    def get_joint_torques(self, desired, current, contact):
        ...
        C = self.robot.getCoriolisAndGravityForces()
        q_dot = current['joint']['vel']
        tau_g = self.robot.getGravityForces()

        #Function
        q_ddot = compute_robot_dynamics(M, C, tau_g, np.zeros(self.dofs))

        #Function // Task (compute on Qk)
        x_dot, x_ddot, Q_k = compute_task_space_dynamics(J['com'], Jdot['com'], inertia_matrix, C, q_dot, q_ddot)

        #Function // Task
        M_k, T_k = compute_task_space_matrices(J['com'], inertia_matrix)

        #Function
        C_eq = compute_robot_dynamics_with_external_forces(
            inertia_matrix, C, tau_g, self.joint_selection, np.zeros(self.dofs), np.zeros(self.dofs), J['com'], np.zeros(6)
        )
        ...
        self.qp_solver.set_values(H, F, A_eq, b_eq, C_eq, A_ineq, b_ineq)
